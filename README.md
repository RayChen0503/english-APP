...
- 真實資料功能：已做到本機帳號、學生管理、狀態保存；尚未接正式後端。
- 真 AI 功能：已做到 AI 診斷/回饋/接力摘要模擬；尚未串接真 AI API。
- 老師/志工端：已做到學生列表、學生詳細、待辦可標記、接力優先序；尚未做多人協作。
- 學生端學習：已擴充多題型、任務契約、反思、復原任務；尚未接正式題庫。
- UI：已補更多狀態卡、流程頁與操作入口；尚未進入正式設計系統。
- 離線同步：已做到本機同步中心與待上傳模擬；尚未做真下載與網路偵測。
